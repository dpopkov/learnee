        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                ...
                const objectMessage = function(message) {
                    const log = $('<div>');
                    const date = message.timestamp == null ? '' : moment.unix(message.timestamp).format('h:mm:ss a');
                    if(message.user != null) {
                        const c = message.user === username ? 'user-me' : 'user-you';
                        log.append($('<span>').addClass(c).text(date+' '+message.user+':\xA0'))
                ...
                server.onmessage = function(event) {
                    if(event.data instanceof ArrayBuffer) {
                        const message = JSON.parse(String.fromCharCode.apply(
