package learn.ee.pj4wcustsupport.tag;
...
public class FormatDateTag extends TagSupport {
    ...
    public int doEndTag() throws JspException {
        if (value == null) {
            if (this.var != null) {
                pageContext.removeAttribute(this.var, scope);
            }
        ...
        if (locale != null) {
            ...
            if (value instanceof Date) {
                formatted = formatDate((Date) value, locale);
            } else if (value instanceof Calendar) {
                formatted = formatDate(((Calendar) value).getTime(), locale);
            } else if (value instanceof TemporalAccessor) {
                formatted = formatDate((TemporalAccessor) value, locale);
            } else {
                throw new JspTagException("Unsupported date type " + value.getClass().getCanonicalName() + ".");
            }
        } else {
        ...
        if (this.var != null) {
            pageContext.setAttribute(this.var, formatted, scope);
        } else {
            try {
    ...
    private Locale getLocale() throws JspTagException {
        try {
            return (Locale) GET_LOCALE_METHOD.invoke(null, pageContext, this, true, true);
    ...
    private TimeZone getTimeZone() throws JspTagException {
        try {
            return (TimeZone) GET_TIME_ZONE_METHOD.invoke(null, pageContext, this);
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new JspTagException(e.toString(), e);
        }
    }
